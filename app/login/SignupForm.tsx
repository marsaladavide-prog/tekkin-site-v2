...
export default function SignupForm() {
  ...
  async function handleGoogle() {
    ...
    try {
      ...
      setError(err?.message || "Registrazione con Google non riuscita");
      setLoading(false);
    }
  }
  ...
  return (
    <div className="w-full max-w-md rounded-xl border border-tekkin-border bg-black/60 p-6 shadow-xl">
      <div className="mb-6 text-center">
        <h1 className="text-xl font-bold text-white">Crea account</h1>
        <p className="text-sm text-gray-400">
          Registrati con email e password oppure usa Google.
        </p>
      </div>
      ...
      <form className="space-y-4" onSubmit={handleSignup}>
        ...
        {info && <p className="text-xs text-tekkin-primary">{info}</p>}
      ...
      <p className="mt-3 text-center text-sm text-gray-400">
        Hai già un account?
        <Link href="/login" className="text-tekkin hover:underline">
          {" "}
          Fai login
        </Link>
      </p>

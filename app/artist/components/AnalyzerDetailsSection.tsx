...
function normalizeBandStatus(raw: unknown): ReferenceAiBandStatus | null {
  ...
  const value =
    typeof candidate.value === "number" && Number.isFinite(candidate.value)
      ...
      : null;
  const target_min =
    typeof candidate.target_min === "number" && Number.isFinite(candidate.target_min)
      ? candidate.target_min
      : null;
  const target_max =
    typeof candidate.target_max === "number" && Number.isFinite(candidate.target_max)
      ? candidate.target_max
      : null;
  const status =
    typeof candidate.status === "string"
      ? (candidate.status as ReferenceAiBandStatus["status"])
      : "in_target";
            ...
            <div className="mt-3 grid gap-3 md:grid-cols-3">
              <div className="rounded-lg border border-white/15 bg-white/5 px-3 py-2 text-[11px]">
                ...
                <p className="text-sm font-semibold text-white">
                  {harmonicBalance ? `${harmonicBalance.tilt_db.toFixed(2)} dB` : "n.a."}
                </p>
                <p className="text-[10px] text-white/60">
              ...
              <div className="rounded-lg border border-white/15 bg-white/5 px-3 py-2 text-[11px]">
                ...
                <p className="text-sm font-semibold text-white">
                  {harmonicBalance ? `${harmonicBalance.low_end_definition.toFixed(2)}%` : "n.a."}
                </p>
                <p className="text-[10px] text-white/60">
              ...
              <div className="rounded-lg border border-white/15 bg-white/5 px-3 py-2 text-[11px]">
                ...
                <p className="text-sm font-semibold text-white">
                  {harmonicBalance ? `${harmonicBalance.hi_end_harshness.toFixed(1)}%` : "n.a."}
                </p>
                <p className="text-[10px] text-white/60">
                  Indica il rischio di alte aggressive o affaticanti sopra gli 8 kHz.
                </p>
              </div>
            </div>
        ...
        <div className="mt-4 grid gap-3 md:grid-cols-3">
          <div className="rounded-xl border border-white/12 bg-black/80 px-3.5 py-3">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[10px] text-white/55">Target: -8.5 / -7.0</span>
            </div>
          ...
          <section className="mt-4">
            ...
            <div className="rounded-xl border border-border/60 bg-black/40 p-3 text-xs flex flex-col gap-3">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  {refAi.bands_in_target != null && refAi.bands_total != null && (
                ...
                <div className="text-right">
                  ...
                  <p className="mt-0.5 text-[11px] text-white/60">{matchDescription}</p>
                  {readiness.reasons.length > 0 && (
                    <p className="mt-1 text-[10px] opacity-60">{readiness.reasons[0]}</p>
                  )}
                </div>
              </div>
              ...
              <div className="flex flex-wrap items-center gap-3 text-[11px] opacity-80">
                <span>
                  Loudness in target:{" "}
                  <span className="font-medium">{refAi.lufs_in_target ? "si" : "no"}</span>
                </span>
                <span>
                  Crest in target:{" "}
                  <span className="font-medium">{refAi.crest_in_target ? "si" : "no"}</span>
                </span>
              </div>
            </div>
            ...
            {refAi.bands_status && (
              ...
              {Object.entries(refAi.bands_status).map(([band, infoRaw]) => {
                const bandInfo = normalizeBandStatus(infoRaw);
                const value = bandInfo?.value ?? null;
                const status = bandInfo?.status ?? "in_target";

                    return (
                      <div key={band} className="flex flex-col items-center">
                        <span className="uppercase text-[10px] opacity-70">{band}</span>
                        ...
                        >
                          {status === "in_target" ? "ok" : status === "low" ? "low" : "high"}
                        </span>
                      </div>
                    );
                  })}
              ...
              {warnings.map((warning, index) => (
                ...
                >
                  <div className="flex items-center justify-between gap-2">
                    <p className="text-sm font-semibold text-white">{warning.message}</p>
                    <span

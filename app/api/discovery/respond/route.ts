...
export async function POST(req: NextRequest) {
  try {
    ...
    // 2) solo il receiver puÃ² rispondere
    if (row.receiver_id !== user.id) {
      return NextResponse.json({ error: "Non autorizzato" }, { status: 403 });
    }

    // 3) idempotenza: se giÃ  gestita, NON Ã¨ un errore
    ...
    // se qualcuno lâha giÃ  gestita tra read e update, torno 200 idempotente
    if (!updated) {
    ...
    // 5) se accept su collab, aggiungo il receiver come collaboratore
    if (action === "accept" && updated.kind === "collab" && updated.project_id && updated.receiver_id) {
      ...
      if (collabErr) {
        console.error("[discovery][respond] project_collaborators insert error", collabErr);
      } else {
        console.log(`[discovery][respond] collaboratore aggiunto: ${updated.receiver_id} al progetto ${updated.project_id}`);
      }
    }

    // 6) se accept, posso rivelare il sender profile
    ...
    if (action === "accept" && sender) {
      ...
      const identityMessage =
        sender.artist_name && sender.artist_name.trim()
          ? `Identità rivelata: ${sender.artist_name}`
          : "Identità rivelata";
      const { error: messageErr } = await admin.from("discovery_messages").insert({
      ...
      if (messageErr) {
        console.error("[discovery][respond] identity message insert error", messageErr);
      }
    }

    // Notifica con notify
    const responseType = `signal_${updated.kind}_${updated.status}`;
    const responseTitle =
    ...
    const responseBody =
      updated.status === "accepted"
        ? updated.kind === "promo"
          ? "Il tuo Signal promo è stato accettato."
          : "Il tuo Signal collab è stato accettato."
        : updated.kind === "promo"
        ? "Il tuo Signal promo è stato rifiutato."
        : "Il tuo Signal collab è stato rifiutato.";

...
def _band_rms(signal: np.ndarray, sr: int, fmin: float, fmax: float) -> float:
    S = np.abs(librosa.stft(signal, n_fft=2048, hop_length=512))
    freqs = librosa.fft_frequencies(sr=sr, n_fft=2048)
    mask = (freqs >= fmin) & (freqs < fmax)
    if not np.any(mask):
        return 1e-6
    band_power = np.mean(S[mask, :] ** 2)
    return math.sqrt(max(band_power, 1e-12))


def _clamp_db(value: float, min_db: float = -24.0, max_db: float = 24.0) -> float:
    return float(max(min_db, min(max_db, value)))
...
def analyze_stems_balance(y: np.ndarray, sr: int) -> StemsBalanceMetrics:
    ...
    """
    mix_rms_val = rms(y)
    mix_rms_db = db_safe(mix_rms_val)

    low_rms = _band_rms(y, sr, 20, 120)
    kick_rms = _band_rms(y, sr, 40, 120)
    lowmid_rms = _band_rms(y, sr, 120, 400)
    mid_rms = _band_rms(y, sr, 400, 2000)
    high_rms = _band_rms(y, sr, 2000, 8000)
    clamp_rms = _band_rms(y, sr, 800, 4000)
    clap_rms = _band_rms(y, sr, 1000, 8000)

    kick_db = db_safe(kick_rms)
    bass_db = db_safe(lowmid_rms)
    drums_db = db_safe(mix_rms_val)
    vocal_db = db_safe(clamp_rms)

    # rapporti
    kick_vs_bass = _clamp_db(
        20.0
        * math.log10(
            max(kick_rms, 1e-12) / max(lowmid_rms, 1e-12)
        )
    )
    vocal_vs_mix = _clamp_db(
        20.0 * math.log10(max(clamp_rms, 1e-12) / max(mix_rms_val, 1e-12))
    )

    clap_vs_kick = _clamp_db(
        20.0 * math.log10(max(clap_rms, 1e-12) / max(kick_rms, 1e-12))
    )
    ...
    return StemsBalanceMetrics(
        ...
        bass_vs_mix_low_db=float(
            _clamp_db(
                20.0
                * math.log10(
                    max(lowmid_rms, 1e-12) / max(low_rms, 1e-12)
                )
            )
        ),
    )
...
def analyze_mix_v1(
    ...
    if integrated_lufs is not None:
        loud.integrated_lufs = float(integrated_lufs)
    if loudness_stats:
        attr_map = {
            "short_term_min": "short_lufs_min",
            "short_term_max": "short_lufs_max",
        }
        for attr_name, key in attr_map.items():
            value = loudness_stats.get(key)
            if value is not None:
                setattr(loud, attr_name, float(value))
